...
import { quoteDefinition } from '$utils/definitions';
...
export const POST: Action = async ({ request }) => {
	const body: DocumentBody<Quote> = await request.json();
	...
	const documentDefinition = quoteDefinition(data);

	const document = printer.createPdfKitDocument(documentDefinition);
	document.pipe(createWriteStream(`${getStaticPath()}/quotes/${quoteNumber}.pdf`));
	document.end();
